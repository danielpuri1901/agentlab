...
# same `agentlab` ECR repository as the main image, tagged with the current
# git short SHA plus a "-video" suffix so the two images can never collide
...
# "-video" tag suffix, and the .tfvars file/variable name this writes.
#
...
# a mutable :latest-video tag (only git-short-SHA-video tags, for the same
...
IMAGE_TAG="${IMAGE_TAG:-$(git rev-parse --short HEAD)-video}"
