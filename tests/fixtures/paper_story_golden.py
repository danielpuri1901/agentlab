...
class PaperStory(StoryScene):
    def beat_1(self):
        self.house = Rectangle(width=4.6, height=3.2, stroke_color=GREY_B).shift(
            LEFT * 3.2 + UP * 0.6
        )
        self.furniture = (
            VGroup(
            ...
            .arrange_in_grid(rows=3, buff=0.25)
            .move_to(self.house)
        )
        self.passport = RoundedRectangle(
            width=0.5,
            height=0.34,
            corner_radius=0.05,
            fill_color=GOLD,
        ...
        self.case = Rectangle(width=1.7, height=1.1, stroke_color=GOLD).shift(
            RIGHT * 3.6 + UP * 0.3
        )
        house_label = self.label("house = the chat", size=22).next_to(
            self.house, UP, buff=0.15
        )
        case_label = self.label("suitcase = the budget", size=22).next_to(
            self.case, UP, buff=0.15
        )
        self.play(
            Create(self.house),
            FadeIn(self.furniture),
            FadeIn(self.passport),
            run_time=1.6,
        )
        self.play(
            Create(self.case), FadeIn(house_label), FadeIn(case_label), run_time=1.2
        )
        self.hold(0.5)

    def beat_2(self):
        self.list_label = self.label(
            'packing list: "summarize concisely"', size=24
        ).to_edge(UP, buff=0.5)
    ...
    def beat_3(self):
        new_label = self.label(
            'packing list: "list every code first"', size=24
        ).to_edge(UP, buff=0.5)
        self.play(
            Transform(self.list_label, new_label), FadeOut(self.sofa), run_time=0.9
        )
        self.play(
            self.passport.animate.set_opacity(1.0).move_to(self.case), run_time=1.3
        )
        check = Line(
            LEFT * 0.15 + DOWN * 0.05,
            RIGHT * 0.05 + DOWN * 0.25,
            color=GREEN,
            stroke_width=6,
        )
        check2 = Line(
    ...
    def beat_4(self):
        ...
        bad_bar = Rectangle(
            width=0.7, height=0.04, fill_color=RED, fill_opacity=1, stroke_width=0
        )
        good_bar = Rectangle(
            width=0.7, height=0.04, fill_color=GOLD, fill_opacity=1, stroke_width=0
        )
        bars = (
            VGroup(bad_bar, good_bar)
        ...
        self.play(
            good_bar.animate.stretch_to_fit_height(1.3, about_edge=DOWN),
            count_up,
            run_time=2.0,
        )
    ...
    def beat_5(self):
        ...
        boundary_label = self.label("50 items tested", size=18, color=GREY_B).next_to(
            boundary, RIGHT, buff=0.1
        )
        self.play(Create(boundary), FadeIn(boundary_label), run_time=1.2)
        question = self.label(
            "Does your compaction prompt name the codes first?",
            size=26,
            color=WHITE,
            width=40,
        )
        ...
        self.play(
            FadeIn(question),
            self.case.animate.set_stroke(ACCENT, width=4),
            run_time=1.4,
        )
        self.hold(0.6)
